...
function Login() {
  ...
  return <Box p={5} shadow="md" borderWidth="1px" width="100%">
    <Heading fontSize="xl" mb="5">登录</Heading>
    <Tabs>
      <TabList>
        <Tab>邮箱登录</Tab>
        <Tab>Token 登录</Tab>
            ...
            <FormControl id="email">
              ...
              <InputGroup size="md">
                ...
                <InputRightElement width="6rem" mx="0.5rem">
                  <Button size="sm" h="1.75rem" onClick={sendCode} isLoading={tokenSent}>
                    发送验证码
                  </Button>
                </InputRightElement>
              </InputGroup>
              <FormHelperText>您所使用的匿名社区可能对邮箱后缀进行限制。</FormHelperText>
            </FormControl>
            <FormControl id="otpToken">
              <FormLabel>验证码</FormLabel>
              <Input value={otpToken} onChange={(event) => setOtpToken(event.target.value)} />
              <FormHelperText>您可以通过邮箱查收验证码邮件。</FormHelperText>
            </FormControl>
            <FormControl id="proxyServer">
              <FormLabel>RPC 服务器</FormLabel>
              <Input value={rpc} onChange={(event) => setRpc(event.target.value)} />
              <FormHelperText>您的所有消息（包括用户令牌、发送与接收的内容）都将通过 RPC 服务器传输。务必确保您信任 RPC 服务器的提供者。</FormHelperText>
            </FormControl>
            <Button
              mt={4}
              colorScheme="blue"
        ...
        <TabPanel>
          <Stack spacing="3">
            ...
            <FormControl id="proxyServer">
              <FormLabel>RPC 服务器</FormLabel>
              <Input value={rpc} onChange={(event) => setRpc(event.target.value)} />
              <FormHelperText>您的所有消息（包括用户令牌、发送与接收的内容）都将通过 RPC 服务器转发。务必确保您信任 RPC 服务器的提供者。</FormHelperText>
            </FormControl>
            <Button
              mt={4}
              colorScheme="blue"
