...
import { Stack, Box, Heading, Text, HStack, Button, Divider, useToast } from "@chakra-ui/react"
import {
    Route,
    NavLink
...
function Navbar() {
    ...
    useEffect(() => {
        ...
        sendRequest().then(() => {}).catch(err => toast({
            title: "无法获取 RPC 后端信息",
            description: `${err}`,
            status: "error",
            duration: 5000,
    ...
    return <Stack spacing={1}>
        ...
        <NavButton exact to="/">首页</NavButton>
        <NavButton to="/trend">趋势</NavButton>
        <Divider></Divider>
        <Box px="5">
            <Text color="gray.500" fontSize="sm">个人</Text>
        </Box>
        ...
        <Divider></Divider>
        <Box px="5">
            <Text color="gray.500" fontSize="sm">系统</Text>
        </Box>
        <NavButton to="/login">{token ? '切换用户': '登录'}</NavButton>
        <Box pt="5">
            <Text color="gray.500" fontSize="xs">
                「闷声发大财」是一个通用的匿名社区前端。
                使用「闷声发大财」访问社区内容，即意味着您同意所使用 RPC 后端的服务条款，且同意对应匿名社区的社区规范与服务条款。
            </Text>
            <Text color="gray.500" fontSize="xs" mt="3">
                您正在使用 {rpc == '/' ? window.location.hostname : rpc} 作为「闷声发大财」的 RPC 后端。
                该 RPC 后端由 {backend?.name || '<无法获取信息>'} 提供服务。
            </Text>
        </Box>
    </Stack>
}
